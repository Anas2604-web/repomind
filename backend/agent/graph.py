    ...
    for attempt in range(max_retries):
        try:
            ...
            for chunk, metadata in agent.stream(
                ...
                if isinstance(chunk, ToolMessage) and isinstance(chunk.content, str):
                    all_tool_content += chunk.content + "\n"
            ...
            citations = []
            seen = set()
            search_text = all_tool_content + "\n" + full_text
            for match in re.finditer(r"FILE: (.+?) \(lines (\d+)-(\d+)\)", search_text):
